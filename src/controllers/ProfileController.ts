...
export class ProfileController {
	static updateProfile = async (req: Request, res: Response) => {
		try {
			...
			if (
				phoneExists &&
				phoneExists._id.toString() !== user._id.toString()
			) {
				throw new RequestConflictError(
					"Este Teléfono ya esta Registrado",
				);
			}
	...
	static updateExtraInfo = async (req: Request, res: Response) => {
		try {
			const user = req.user as UserInterface;
			const { country, region, city, province, reference, postalCode } =
				req.body;
			...
			res.status(200).json({
				message: "Información Adicional Actualizada Exitosamente",
				user,
			});
		} catch (error) {
			throw new InternalServerError();
