...
export class AuthController {
	static createAccount = async (req: Request, res: Response) => {
		...
		res.status(201).json({
			message:
				"Usuario Creado Exitosamente, Hemos enviado su solicitud de verificación.",
		});
	};
	...
	static createPassword = async (req: Request, res: Response) => {
		...
		(await user.save(), await tokenRecord.deleteOne());

		res.status(200).json({
			message: "Contraseña Establecida Exitosamente, inicie sesión",
		});
	};
	...
	static forgotPassword = async (req: Request, res: Response) => {
		...
		if (!user.passwordSet) {
			...
			res.status(200).json({
				message:
					"Tu Contraseña no esta Establecida, recibiras un correo para establecerla",
			});
		} else {
			// Generate a password reset token
			const passwordResetToken = await Token.create({
			...
			res.status(200).json({
				message: "Recibiras un correo para establecer tu contraseña",
			});
		}
	};
	...
	static resetPassword = async (req: Request, res: Response) => {
		...
		res.status(200).json({
			message: "Contraseña Reestablecida Exitosamente, inicie sesión",
		});
	};
}
